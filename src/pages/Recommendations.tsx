...
import { Lightbulb, Zap, BookOpen, Target } from "lucide-react";
...
export default function Recommendations() {
  ...
  const tips = [
    { title: "Focus on weak areas", desc: lowProgress.length > 0 ? `"${lowProgress[0]?.title}" needs attention — you're at ${lowProgress[0]?.progress}%.` : "You're on track across all courses!", icon: Zap },
    { title: "Unlock next level", desc: locked.length > 0 ? `Complete current courses to unlock ${locked.length} more.` : "All courses unlocked! 🎉", icon: BookOpen },
    { title: "Daily compound growth", desc: "Complete at least 1 chapter daily — small consistent actions compound into mastery.", icon: Target },
  ];
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-8 finance-grid min-h-screen">
      <motion.header initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-medium text-accent uppercase tracking-wider">Smart Insights</span>
        </div>
        <h1 className="text-2xl md:text-3xl font-heading font-bold">Personalized For You</h1>
        <p className="text-muted-foreground mt-1">Recommendations based on your financial learning patterns</p>
      </motion.header>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {tips.map((tip, i) => (
            <motion.div
              ...
              className="rounded-2xl border border-accent/20 bg-accent/5 p-5 space-y-2 relative overflow-hidden"
            >
